...
export function renderAvatar(character, rankId = 0, size = "md", opts = {}) {
  ...
  const outfit =
    opts.outfitLabel ||
    (gender === "female"
      ? ["米白短襖", "青綠長衫", "白衣青袍", "青袍官服", "青袍玉印", "鳳冠藍袍", "紅甲鳳氅", "鳳袍珠冠"][rank]
      : ["米白短衣", "青綠長衫", "白衣青袍", "青袍官服", "青袍玉印", "藍袍玉帶", "紅甲金冠", "龍袍冕旒"][rank]);
  ...
  if (stageArt) {
    ...
    <div class="avatar-art avatar-${size} outfit-${rank} stage-face" style="--accent:${accent};--glow:${GLOW[Math.min(rank, GLOW.length - 1)]};width:${dims}px;height:${h}px" role="img" aria-label="${character.name}">
      <img src="${stageArt.src}?v=rad26" alt="${character.name}" width="${dims}" height="${h}" loading="lazy" />
    </div>`;
  }
  ...
  if (character.portrait) {
    ...
    <div class="avatar-art avatar-${size} outfit-${rank}" style="--accent:${accent};--glow:${GLOW[Math.min(rank, GLOW.length - 1)]};width:${dims}px;height:${h}px" role="img" aria-label="${character.name}">
      <img src="${character.portrait}?v=rad26" alt="${character.name}" width="${dims}" height="${h}" loading="lazy" />
    </div>`;
  }
